  ...
  function homeController($scope,$timeout,$http,$state,$rootScope,perfilService){
      ...
      function getVentas(){        
          ...
          $scope.totalNegociacion=0;
                  ...
                  for(i=0;i<$scope.ventas.length;i++){
                    ...
                    $scope.totalIdea=addCommas($scope.totalIdea);
                    $scope.totalContacto=addCommas($scope.totalContacto);
                    $scope.totalNecesidad=addCommas($scope.totalNecesidad);
                    $scope.totalPropuesta=addCommas($scope.totalPropuesta);
                    $scope.totalNegociacion=addCommas($scope.totalNegociacion);
                  console.log("ventas");
                  console.log(data);
                    }).error(function(){
                      alert("an unexpected error ocurred")
     ...
     function calcularTotal(producto){
      ...
      for(i=0;i<$scope.productos.length;i++){
        $scope.NuevoNegocio.total=($scope.productos[i].precio*$scope.productos[i].cantidad)+$scope.NuevoNegocio.total;
      }
        ...
        if($scope.productos[i].isSelected){
          if($scope.productos[i].cantidad==undefined || $scope.productos[i].cantidad > 999999 || (!/^([0-9])*$/.test($scope.productos[i].cantidad)  )){
            ...
            $scope.errorProductos="Ingresa una cantidad válida";
                    ...
                    $scope.productos[i].existencias=$scope.productos[i].existencias-$scope.productos[i].cantidad;
